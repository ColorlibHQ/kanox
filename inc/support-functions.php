...
// social media
if ( ! function_exists( 'kanox_social' ) ) {
	function kanox_social( $args = array()  ){
		...
		$url = kanox_opt('kanox_social_url');
...
function kanox_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'kanox' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'kanox' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'kanox' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function kanox_get_post_views($postID){
    $count_key = 'kanox_post_views_count';
...
function kanox_set_post_views($postID) {
    $count_key = 'kanox_post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if($count==''){
        $count = 0;
...
// blog post categoty 
function kanox_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'kanox' ) );
